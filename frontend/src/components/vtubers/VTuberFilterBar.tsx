...
import { Search, X, Gamepad, Music, Palette, Mic, Headphones, MessageSquare, Sparkles } from '@/components/ui/Icons';
import { ContentTypeIcon } from './VTuberIDCard';
...
export default function VTuberFilterBar({
  search,
  onSearchChange,
  contentType,
  onCategoryChange,
    ...
    >
        ...
        >
          <span
            style={{
              ...
              animation: onlyLive ? 'vtuber-pulse-dot 1.4s infinite ease-in-out' : 'none',
            }}
          />
          Solo En Vivo
        </button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '4px', maxWidth: '100%' }} className="hide-scrollbar">
          {CATEGORIES.map(cat => {
            ...
            return (
              ...
              >
                {cat.icon && <ContentTypeIcon type={cat.icon} size={13} color={isSelected ? 'var(--primary-hover)' : 'var(--text-muted)'} />}
                <span>{cat.label}</span>
              </button>
            );
          })}
        ...
        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 600, flexShrink: 0 }}>
          {totalResults} {totalResults === 1 ? 'VTuber' : 'VTubers'}
        </div>
      </div>
    </div>
  );
