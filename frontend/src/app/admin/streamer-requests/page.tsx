...
import { STREAMER_SURVEY_QUESTIONS, type SurveyAnswers } from '@gremio-estelar/shared';
...
interface StreamerRequest {
  ...
  user: {
    ...
    username: string;
    email: string;
    streamerProfile: { id: string; displayName: string; avatarUrl: string | null; isApproved: boolean; isVerified: boolean } | null;
  };
...
export default function AdminStreamerRequestsPage() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<StreamerRequest | null>(null);
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/admin/streamer-requests?${params}`);
  ...
  const handleApprove = async (id: string) => {
    ...
    setProcessing(true);
    try {
      const res = await apiFetch(`/admin/streamer-requests/${id}/approve`, { method: 'POST' });
  ...
  const handleReject = async (id: string) => {
    ...
    setProcessing(true);
    try {
      await apiFetch(`/admin/streamer-requests/${id}/reject`, {
        method: 'POST',
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 700, marginBottom: '8px' }}>Solicitudes Streamer</h1>
          <p style={{ color: 'var(--text-muted)' }}>Revisa y aprueba las solicitudes de nuevos Streamers oficiales</p>
        </div>
      </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                    {['Solicitante', 'Nombre Streamer', 'Email', 'Estado', 'Fecha', 'Acciones'].map(h => (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }} onClick={() => { setSelectedRequest(null); setRejectNotes(''); }}>
          <div className="glass" style={{ padding: '32px', borderRadius: '20px', width: '100%', maxWidth: '520px' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
              ...
              <div>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '2px' }}>Nombre Streamer</div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {(() => {
                      try {
                        ...
                        return STREAMER_SURVEY_QUESTIONS.filter((q) => answers[q.id]).map((q) => (
                            <div key={q.id} style={{
