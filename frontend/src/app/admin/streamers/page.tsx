...
export default function AdminVtubersPage() {
  ...
  // backend (PATCH /admin/streamers/:id exige authorize(ADMIN)). Los moderadores
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/admin/streamers?${params}`);
      setData(res);
    } catch (err: unknown) {
      setFetchError(true);
  ...
  const fetchUnverified = async () => {
    ...
    try {
      const res = await apiFetch('/admin/streamers?isVerified=false&limit=10');
  ...
  const fetchUnapproved = async () => {
    ...
    try {
      const res = await apiFetch('/admin/streamers?isApproved=false&limit=10');
  ...
  const saveProfile = async () => {
    ...
    setSaving(true);
    try {
      await apiFetch(`/admin/streamers/${selectedProfile.id}`, { method: 'PATCH', body: JSON.stringify(editData) });
  ...
  const handleToggleFlag = async (id: string, field: string, currentValue: boolean, label: string) => {
    ...
    try {
      await apiFetch(`/admin/streamers/${id}`, { method: 'PATCH', body: JSON.stringify({ [field]: !currentValue }) });
      showToast(`Streamer ${label.toLowerCase()} ${currentValue ? 'desactivado' : 'activado'}`, 'success');
      fetchData();
      fetchUnverified();
      fetchUnapproved();
    } catch (err: unknown) { showToast(err instanceof Error ? err.message : 'Error', 'error'); }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px', flexWrap: 'wrap', gap: '8px' }}>
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 700, margin: 0 }}>Streamers</h1>
          <p style={{ color: 'var(--text-muted)', marginTop: '4px', marginBottom: 0 }}>Gestión de perfiles Streamer</p>
        </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '2px 0 0' }}>
                Estos Streamers están aprobados pero aún no verificados. Haz clic en ✓ para verificarlos.
              </p>
            </div>
          </div>
      ...
      {!loadingUnverified && quickVerifyError && (
        <div className="glass" style={{ padding: '24px', borderRadius: '16px', marginBottom: '24px', textAlign: 'center', border: '1px solid rgba(244,67,54,0.2)' }}>
          <div style={{ color: '#f44336', fontSize: '0.9rem', marginBottom: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            ...
            No se pudieron cargar los Streamers pendientes de verificar
          </div>
          <button
        ...
        <div className="glass" style={{ padding: '20px', borderRadius: '16px', marginBottom: '24px', border: '1px solid rgba(33,150,243,0.15)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '2px 0 0' }}>
                Estas solicitudes están pendientes de aprobación. Al aprobar, se les otorgará el rol Streamer.
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      ...
      <div className="glass" style={{ borderRadius: '16px', overflow: 'hidden' }}>
        {loading ? (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>Cargando Streamers...</div>
        ) : fetchError ? (
          ...
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>No se encontraron Streamers</div>
        ) : (
          <>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10000, padding: '20px', overflowY: 'auto' }} onClick={() => setSelectedProfile(null)}>
          <div className="glass" style={{ padding: '32px', borderRadius: '20px', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div>
                <h2 style={{ fontSize: '1.3rem', fontWeight: 700, margin: 0 }}>Editar Perfil Streamer</h2>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div className="form-group">
                      ...
                      <select className="input" value={editData.contentType} onChange={e => setEditData({ ...editData, contentType: e.target.value })}>
                        ...
                        <option value="vtuber">✨ Streamer General</option>
                      </select>
                    </div>
                    <div className="form-group">
